...
- [x] 외부 사용과 수정·재배포를 허용하지 않기로 결정한다.
- [x] 오픈 소스 특허 조항을 부여하지 않기로 결정한다.
- [x] copyleft 라이선스를 적용하지 않기로 결정한다.
- [x] 별도 서면 허가 없는 상업적 사용을 허용하지 않기로 결정한다.
- [ ] 향후 외부 기여를 받을 경우 권리 귀속과 Contributor License Agreement 필요 여부를 다시 결정한다.
- [ ] 아래 선택지 중 프로젝트 의도에 맞는 방식을 검토한다.
  ...
  - [x] 비공개 권리 유지: `LICENSE`를 추가하지 않고 사용 허가를 부여하지 않는다.
  - [ ] 별도 상용 라이선스: 법률 검토 후 프로젝트 전용 조건을 사용한다.
- [ ] 불확실하거나 제3자 권리가 섞여 있으면 법률 전문가에게 검토를 요청한다.
- [x] 선택 결과에 따라 루트 `LICENSE`를 추가하지 않는다.
- [ ] README 저작권 고지에 사용할 정확한 저작권자 이름과 연도를 확인한다.
- [x] `README.md`에 추가 사용 권한을 부여하지 않는다는 점을 명시한다.
