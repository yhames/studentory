...
이 저장소에는 오픈 소스 라이선스를 부여하지 않으며 `LICENSE` 파일을 두지 않는 것은 의도된 정책입니다. 별도의 서면 허가가 없는 한 저장소 콘텐츠를 복제, 수정, 배포하거나 파생물을 만드는 권한은 부여되지 않습니다.

공개 저장소의 열람과 GitHub 내 fork에는 GitHub Terms of Service가 적용됩니다. 저장소 공개는 그 범위를 넘어서는 추가 사용 허가를 의미하지 않습니다.

적용 범위, 에셋 출처와 의존성 감사 결과는 [docs/COPYRIGHT_AND_ASSETS.md](docs/COPYRIGHT_AND_ASSETS.md)를 확인합니다. 외부 패키지에는 각 패키지의 자체 라이선스가 적용됩니다.
